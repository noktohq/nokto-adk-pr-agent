...
This repository is the new Google ADK control plane for secure PR task preparation.

- Preserve `MODEL = "gemini-3.7-flash"` through `app.project.MODEL`.
- Keep the hosted agent read-only. Do not add repository mutations or orchestrator dispatch without a separately approved design and human-in-the-loop gate.
- Keep `app.task_contract` compatible with the strict Zod contract documented by `nokto-agent-orchestrator/src/types.ts`.
- Every new tool must be added deliberately to `app.guardrails.READ_ONLY_TOOLS` and must have unit tests for invalid input and failure behavior.
- Never log or return `GITHUB_TOKEN`.
- Update `README.md`, `docs/architecture.md`, tests, and eval cases when behavior changes.
- Run `agents-cli lint`, `uv run pytest tests/unit`, and the relevant live integration checks before release.
